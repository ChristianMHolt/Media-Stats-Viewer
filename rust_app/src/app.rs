...
impl MediaStatsApp {
    ...
    fn compare_items(&self, a: &MediaItem, b: &MediaItem, col: SortColumn, config: &AppConfig) -> std::cmp::Ordering {
        match col {
    ...
    fn apply_filter_and_sort(&mut self) {
        ...
        // Use a stable sort approach
        if let Some((col, order)) = sort_secondary {
            indices.sort_by(|&a, &b| {
                let item_a = &items[a];
                let item_b = &items[b];
                // We can't reuse self.compare_items easily here due to borrow of self.
                // We must duplicate logic or extract comparator.
                // Duplicating logic for now to avoid fighting borrow checker with helper methods.
                // Actually we can make compare_items static or move it out.
                // But it needs config.
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.horizontal(|ui| {
                if ui.button("Select Library Folder").clicked() {
                    // Need to run this slightly differently to avoid blocking if possible,
                    // but for now synchronous dialog is acceptable in this context.
                ...
                egui::ComboBox::from_id_source("status_sort")
            ...
            // We need to move sort_action into closure
            // But we can't move it out easily unless we use RefCell or just a local var that we can assign to.
            // Since we are in the same scope, we can't mutate `sort_action` inside closure if it's not `RefCell` or `&mut`.
            // But `TableBuilder` closures are callbacks.

            // Workaround: Use a list of actions to apply after

            let mut actions = Vec::new(); // (col, secondary)
                    ...
                    for &idx in filtered {
                        ...
                        body.row(row_height, |mut row| {
                            ...
                            row.col(|ui| {
                                ...
                                let status = config.media_statuses.get(&item.path).map(|s| s.as_str()).unwrap_or("");
